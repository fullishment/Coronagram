...
//        urlBuilder.append("&" + URLEncoder.encode("numOfRows","UTF-8") + "=" + URLEncoder.encode("10", "UTF-8")); /*한 페이지 결과 수*/
		urlBuilder.append("&" + URLEncoder.encode("startCreateDt", "UTF-8") + "="
				+ URLEncoder.encode("20200120", "UTF-8")); /* 검색할 생성일 범위의 시작 */
	...
	/*
	 * @RequestMapping(value = "/weekDataAjax", method = RequestMethod.POST,
	 * produces = "test/xml;charset=UTF-8")
	 * 
	 * @ResponseBody public String weekDataAjax() throws Throwable{
	 * 
	 * LocalTime nowTime = LocalTime.now(); LocalTime limitTime = LocalTime.of(9,
	 * 59, 59); LocalDate now = LocalDate.now(); LocalDate nowm = now.minusDays(1);
	 * LocalDate weekDays = now.minusDays(7); LocalDate weekDaysm =
	 * now.minusDays(8); DateTimeFormatter dtf =
	 * DateTimeFormatter.ofPattern("yyyyMMdd");
	 * 
	 * String today = now.format(dtf); String todaym = nowm.format(dtf);
	 * 
	 * String before6 = weekDays.format(dtf); String before7 =
	 * weekDaysm.format(dtf);
	 * 
	 * 
	 * StringBuilder urlBuilder = new StringBuilder(
	 ...
	 * "=iwZFIlKLh4A5tfy0C89hfMwqm3%2Bfk8nR9HsfaLe%2FBP309oMxAJd2l796xiYG8em2N%2FlwQ%2B7YEETT87zAHSMHbw%3D%3D"
	 * ); Service Key if(nowTime.isBefore(limitTime)) { urlBuilder.append("&" +
	 * URLEncoder.encode("startCreateDt","UTF-8") + "=" + URLEncoder.encode(before7,
	 * "UTF-8")); 검색할 생성일 범위의 시작 }else { urlBuilder.append("&" +
	 * URLEncoder.encode("startCreateDt","UTF-8") + "=" + URLEncoder.encode(before6,
	 * "UTF-8")); 검색할 생성일 범위의 시작 } if(nowTime.isBefore(limitTime)) {
	 * urlBuilder.append("&" + URLEncoder.encode("endCreateDt","UTF-8") + "=" +
	 * URLEncoder.encode(todaym, "UTF-8")); 검색할 생성일 범위의 종료 }else {
	 * urlBuilder.append("&" + URLEncoder.encode("endCreateDt", "UTF-8") + "=" +
	 * URLEncoder.encode(today, "UTF-8")); 검색할 생성일 범위의 종료
	 * 
	 ...
	 * conn.setRequestProperty("Content-type", "application/json");
	 * System.out.println("Response code: " + conn.getResponseCode());
	 * System.out.println("URL: " + urlBuilder.toString()); BufferedReader rd;
	 * if(conn.getResponseCode() >= 200 && conn.getResponseCode() <= 300) { rd = new
